...
final class TomoArAvatarCameraFactory: NSObject, FlutterPlatformViewFactory {
  ...
  func create(
    ...
    TomoArAvatarCameraView(frame: frame, viewId: viewId, messenger: messenger, args: args)
  }
}

private final class TomoArAvatarCameraView: NSObject, FlutterPlatformView, ARSCNViewDelegate {
  private let containerView: UIView
  private let sceneView: ARSCNView
  private let statusLabel = UILabel()
  private let channel: FlutterMethodChannel
  private var avatar: TomoNativeAvatar
  private var filterMode: TomoArCameraFilterMode
  ...
  init(frame: CGRect, viewId: Int64, messenger: FlutterBinaryMessenger, args: Any?) {
    ...
    channel = FlutterMethodChannel(
      name: "tomo/ar_avatar_camera_\(viewId)",
      binaryMessenger: messenger
    )
    avatar = TomoNativeAvatar(arguments: (args as? [String: Any])?["avatar"] as? [String: Any])
    filterMode = TomoArCameraFilterMode(arguments: args as? [String: Any])
    super.init()
  ...
  private func configureChannel() {
    channel.setMethodCallHandler { [weak self] call, result in
      ...
      case "setAvatar":
        ...
        self.avatar = TomoNativeAvatar(arguments: payload)
        self.applyCurrentFilter()
        result(nil)
      case "setFilterMode":
        self.filterMode = TomoArCameraFilterMode(rawValue: call.arguments as? String)
        self.applyCurrentFilter()
        result(nil)
      case "capture":
  ...
  private func capture(result: @escaping FlutterResult) {
    ...
    do {
      let fileName = "tomo_ar_avatar_\(Int(Date().timeIntervalSince1970 * 1000)).jpg"
  ...
  func renderer(_ renderer: SCNSceneRenderer, nodeFor anchor: ARAnchor) -> SCNNode? {
    ...
    if let device = sceneView.device,
      ...
      if let material = faceGeometry.firstMaterial {
        TomoArFilterRenderer.configureFaceMaterial(material, mode: filterMode, avatar: avatar)
      }
    ...
    let overlayNode = SCNNode(geometry: TomoArFilterRenderer.makeOverlayPlane(mode: filterMode, avatar: avatar))
    overlayNode.name = "tomo-avatar-face-overlay"
    overlayNode.position = TomoArFilterRenderer.overlayPosition(for: filterMode)
  ...
  private func applyCurrentFilter() {
    DispatchQueue.main.async { [weak self] in
      ...
      if let material = self.faceGeometryNode?.geometry?.firstMaterial {
        TomoArFilterRenderer.configureFaceMaterial(material, mode: self.filterMode, avatar: self.avatar)
      }
      self.faceOverlayNode?.geometry = TomoArFilterRenderer.makeOverlayPlane(
        mode: self.filterMode,
        avatar: self.avatar
      )
      self.faceOverlayNode?.position = TomoArFilterRenderer.overlayPosition(for: self.filterMode)
    }
  }

}

private enum TomoArCameraFilterMode: String {
  case avatar
...
private struct TomoNativeAvatar: Equatable {
  let skin: Int
  let hair: Int
  let shirt: Int
  let eyes: Int
  ...
  init(arguments: [String: Any]?) {
    skin = TomoNativeAvatar.clamped(arguments?["skin"], max: TomoPalette.skinColors.count)
    hair = TomoNativeAvatar.clamped(arguments?["hair"], max: TomoPalette.hairColors.count)
    shirt = TomoNativeAvatar.clamped(arguments?["shirt"], max: TomoPalette.shirtColors.count)
    eyes = TomoNativeAvatar.clamped(arguments?["eyes"], max: 4)
    mouth = TomoNativeAvatar.clamped(arguments?["mouth"], max: 3)
    accessory = TomoNativeAvatar.clamped(arguments?["accessory"], max: 4)
  ...
  var skinColor: UIColor {
    TomoPalette.skinColors[skin]
  }

  var hairColor: UIColor {
    TomoPalette.hairColors[hair % TomoPalette.hairColors.count]
  }

  var shirtColor: UIColor {
    TomoPalette.shirtColors[shirt]
  }
}

private enum TomoPalette {
...
private enum TomoArFilterRenderer {
  static func configureFaceMaterial(
    _ material: SCNMaterial,
    mode: TomoArCameraFilterMode,
    avatar: TomoNativeAvatar
  ) {
  ...
  static func makeOverlayPlane(mode: TomoArCameraFilterMode, avatar: TomoNativeAvatar) -> SCNPlane {
  ...
  static func overlayPosition(for mode: TomoArCameraFilterMode) -> SCNVector3 {
    SCNVector3(0, 0.002, 0.058)
  }

  private static func overlaySize(for mode: TomoArCameraFilterMode) -> CGSize {
    CGSize(width: 0.216, height: 0.276)
  }

  private static func makeFeatureImage(for avatar: TomoNativeAvatar) -> UIImage {
  ...
  private static func drawFace(_ avatar: TomoNativeAvatar, includeSkin: Bool, in cg: CGContext) {
