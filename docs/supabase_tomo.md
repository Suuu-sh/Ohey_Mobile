...
flutter run \
  ...
  --dart-define=SUPABASE_AUTH_REDIRECT_URL=app.tomo.tomo://login-callback/
```

Production/release builds should override `SUPABASE_URL` and `SUPABASE_PUBLISHABLE_KEY` with the `tomo` project values.

## Configured database/Auth/storage

Both `tomo` and `dev-nomo` are kept on the same app schema:
...
- private Storage bucket: `tomo-photos`

Auth callback URL:

```text
app.tomo.tomo://login-callback/
```

## Important: dev and production must stay separate

`tomo` Supabase and `tomo-backend` are production resources. Do not use them for local/dev checks.
...
# Historical local-backend scripts may exist, but Tomo dev/Simulator checks should use dev Render backend.
```
