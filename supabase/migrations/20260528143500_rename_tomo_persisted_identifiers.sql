-- Rename persisted pre-release Nomo identifiers to Tomo.
-- This keeps already-created dev/prod projects aligned with the Tomo app code.
...
begin
  ...
  values (
    new.id,
    candidate_user_id,
    coalesce(display_name_value, 'Tomo'),
...
set avatar_url = replace(avatar_url, 'nomo_avatar:', 'tomo_avatar:')
where avatar_url like 'nomo_avatar:%';

update public.profiles p
set user_id = 'tomo_' || substr(p.user_id, 6)
where p.user_id like 'nomo\_%' escape '\'
  and not exists (
    select 1
    from public.profiles p2
    where p2.user_id = 'tomo_' || substr(p.user_id, 6)
  );

update public.profiles
set user_id = 'tomo_official',
    display_name = 'Tomo公式'
where user_id = 'nomo_official';
...
drop policy if exists "nomo_photos_owner_delete" on storage.objects;
drop policy if exists nomo_photos_insert_own on storage.objects;
drop policy if exists "nomo_photos_insert_own" on storage.objects;
...
set bucket_id = 'tomo-photos'
where bucket_id = 'nomo-photos';

delete from storage.buckets
where id = 'nomo-photos'
  and not exists (
    select 1
...
create policy tomo_photos_select_visible
  on storage.objects
  for select
  to authenticated
  using (
    bucket_id = 'tomo-photos'
    and (
...
create policy tomo_photos_owner_insert
  on storage.objects
  for insert
  to authenticated
  with check (
...
create policy tomo_photos_owner_update
  on storage.objects
  for update
  to authenticated
  using (
...
create policy tomo_photos_owner_delete
  on storage.objects
  for delete
  to authenticated
  using (
