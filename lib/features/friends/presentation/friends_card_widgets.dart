...
class _FriendCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => TomoFriendUserBlock(
    friend: friend,
...
Future<void> showTomoFriendProfileSheet(
  BuildContext context, {
  required TomoFriend friend,
}) {
...
class _FriendProfileSheetState extends ConsumerState<_FriendProfileSheet> {
  ...
  void _handleSelectedStatusChanged(TomoDailyStatus status) {
  ...
  Future<void> _openActionMenu() async {
    ...
    final action = await showTomoBottomSheet<_FriendProfileAction>(
      context: context,
      useSafeArea: true,
      barrierColor: Colors.black.withValues(alpha: .58),
...
Future<_FriendProfileReportReason?> _selectFriendReportReason(
  ...
  return showTomoBottomSheet<_FriendProfileReportReason>(
    context: context,
    useSafeArea: true,
    barrierColor: Colors.black.withValues(alpha: .58),
        ...
        children: [
          SizedBox(
            ...
            child: Align(
              ...
              child: TomoAvatarView(avatar: avatar, size: 156),
            ),
          ),
          Container(
            width: double.infinity,
...
class _FriendProfileCalendar extends ConsumerStatefulWidget {
  ...
  final TomoFriend friend;
  final _FriendStatus status;
  final ValueChanged<TomoDailyStatus> onSelectedStatusChanged;
      ...
      targets.map((date) async {
        try {
          ...
          return MapEntry(
            ...
            tomoDailyStatusFromKey(friend?.statusKey),
          );
        } catch (_) {
          return MapEntry(
            _friendProfileDateKey(date),
...
Color _friendProfileCalendarStatusTileAccent(TomoDailyStatus status) =>
    switch (status) {
      TomoDailyStatus.available => const Color(0xFFFF5EA8),
      TomoDailyStatus.maybeAvailable => const Color(0xFF20B9FF),
      TomoDailyStatus.dependsOnTime => const Color(0xFF8A62FF),
      TomoDailyStatus.hasPlans => const Color(0xFF738092),
      TomoDailyStatus.unselected => const Color(0xFF9AF21A),
    };
