...
extension _TomoCameraScreenActions on _TomoCameraScreenState {
  Future<void> _capture() async {
    ...
    try {
      ...
      await _closeCamera(
        TomoCameraResult(
          path: outputPath,
          filterName: _TomoCameraScreenState._plainFilterName,
        ),
      );
  ...
  Future<String> _photoPathForSelectedFraming(String path) async {
    return switch (_selectedFraming) {
      _CameraFraming.square => tomoWriteSquarePhotoCopy(path),
      _CameraFraming.landscape => tomoWriteLandscapePhotoCopy(path),
    };
  }
  ...
  Future<String?> _photoPathFromAlbum(String path) async {
    if (_selectedFraming == _CameraFraming.square) {
      return tomoWriteSquarePhotoCopy(path);
    }
    final dimensions = await tomoReadPhotoDimensions(path);
    if (dimensions.isLandscape) return tomoWriteLandscapePhotoCopy(path);
  ...
  void _showSnack(String message) {
    TomoToast.show(context, message);
  }
}
