...
extension _OheyCameraScreenActions on _OheyCameraScreenState {
  Future<void> _capture() async {
    ...
    try {
      ...
      await _closeCamera(
        OheyCameraResult(
          path: outputPath,
          filterName: _OheyCameraScreenState._plainFilterName,
        ),
      );
  ...
  Future<String> _photoPathForSelectedFraming(String path) async {
    return switch (_selectedFraming) {
      _CameraFraming.square => oheyWriteSquarePhotoCopy(path),
      _CameraFraming.landscape => oheyWriteLandscapePhotoCopy(path),
    };
  }
  ...
  Future<String?> _photoPathFromAlbum(String path) async {
    if (_selectedFraming == _CameraFraming.square) {
      return oheyWriteSquarePhotoCopy(path);
    }
