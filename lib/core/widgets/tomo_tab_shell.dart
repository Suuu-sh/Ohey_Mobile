...
import '../../features/camera/presentation/tomo_camera_screen.dart';
...
import '../models/tomo_invite.dart';
import '../models/tomo_user.dart';
import '../theme/app_colors.dart';
import '../theme/tomo_theme_mode.dart';
import 'tomo_3d_button.dart';
import 'tomo_backend_busy_screen.dart';
import 'tomo_bottom_sheet.dart';
import 'tomo_daily_status_3d_option.dart';
import 'tomo_pop_icon.dart';
import 'tomo_toast.dart';

class TomoTabShell extends ConsumerStatefulWidget {
  const TomoTabShell({super.key});

  @override
  ConsumerState<TomoTabShell> createState() => _TomoTabShellState();
}

class _TomoTabShellState extends ConsumerState<TomoTabShell>
  ...
  Future<void> _loadOnboardingPref() async {
    ...
    setState(() {
      ...
      _isOnboardingSeen =
          prefs.getBool(TomoLastAccountStore.onboardingSeenKey) ?? false;
    });
  }
  ...
  Future<void> _setOnboardingSeen() async {
    ...
    await SharedPreferences.getInstance().then(
      (prefs) => prefs.setBool(TomoLastAccountStore.onboardingSeenKey, true),
    );
  }
  ...
  List<Widget> get _pages => [
    ...
    const TomoToastAccent(color: _friendsAccentColor, child: FriendsScreen()),
    TomoToastAccent(
  ...
  Future<void> _openMemoryFlow() async {
    final action = await showTomoBottomSheet<_MemoryStartAction>(
      context: context,
      useSafeArea: true,
      barrierColor: Colors.black.withValues(alpha: .58),
      builder: (_) => TomoToastAccent(
  ...
  void _handleIncomingInvites(List<TomoInvite> invites) {
    final currentUser = ref.read(tomoUserProvider);
    if (currentUser == null ||
        currentUser.dailyStatus == TomoDailyStatus.unselected) {
      return;
    }
    final pendingInvites = invites
        .where((invite) => invite.status == TomoInviteStatus.pending)
  ...
  Future<void> _showIncomingInviteModal(TomoInvite invite) async {
  ...
  void _maybeShowDailyStatusPrompt(TomoUser user) {
    if (user.dailyStatus != TomoDailyStatus.unselected ||
  ...
  Future<void> _showDailyStatusPrompt() async {
    ...
    try {
      ...
      if (mounted &&
          ref.read(tomoUserProvider)?.dailyStatus ==
  ...
  Widget build(BuildContext context) {
    ...
    final pendingFriendRequestsAsync = user == null
        ? const AsyncValue<List<TomoFriendRequestItem>>.data(
            <TomoFriendRequestItem>[],
          )
    ...
    ref.listen<AsyncValue<List<TomoInvite>>>(
    ...
    if (user == null && hasSession && !_didAttemptProfileRestore) {
      _invitePollTimer?.cancel();
      _invitePollTimer = null;
      return const TomoBackendBusyScreen();
    }
...
class _DailyStatusRequiredSheet extends ConsumerStatefulWidget {
  ...
  final Future<void> Function(TomoDailyStatus status) onSelect;
...
class _DailyStatusRequiredSheetState
  ...
  static const _options = <TomoDailyStatus>[
  ...
  Future<void> _select(TomoDailyStatus status) async {
    ...
    try {
      ...
      TomoToast.show(context, '今日は「${status.label}」だね');
    } catch (_) {
      if (!mounted) return;
      setState(() => _savingStatus = null);
      TomoToast.show(context, '設定できなかったよ。もう一度ためしてね');
    }
  }
    ...
    return Semantics(
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Row(
            children: [
              TomoPopIcon(icon: icon, color: color, size: 46, iconSize: 25),
