...
enum TomoToastPlacement { top, bottom }

class TomoToastAccent extends InheritedTheme {
  const TomoToastAccent({super.key, required this.color, required super.child});

  final Color color;

  static Color? maybeOf(BuildContext context) {
    final element = context
        .getElementForInheritedWidgetOfExactType<TomoToastAccent>();
    final widget = element?.widget;
    return widget is TomoToastAccent ? widget.color : null;
  }

  @override
  Widget wrap(BuildContext context, Widget child) {
    return TomoToastAccent(color: color, child: child);
  }

  @override
  bool updateShouldNotify(TomoToastAccent oldWidget) {
...
class TomoToast {
  ...
  static const defaultPlacement = TomoToastPlacement.bottom;
  ...
  static void show(
    ...
    TomoToastPlacement placement = defaultPlacement,
    Color? accentColor,
    String? actionLabel,
    ...
    final resolvedAccentColor = accentColorForIcon(
      icon,
      pageAccentColor: TomoToastAccent.maybeOf(context),
    ...
    entry = OverlayEntry(
      builder: (context) => _TomoToastOverlay(
        message: message,
        icon: icon,
...
class _TomoToastOverlay extends StatefulWidget {
  const _TomoToastOverlay({
  ...
  final TomoToastPlacement placement;
  final Color accentColor;
  final String? actionLabel;
  final FutureOr<void> Function()? onAction;

  @override
  State<_TomoToastOverlay> createState() => _TomoToastOverlayState();
}

class _TomoToastOverlayState extends State<_TomoToastOverlay>
  ...
  void initState() {
    ...
    _slide = Tween<Offset>(
      begin: widget.placement == TomoToastPlacement.bottom
          ? const Offset(0, 1.18)
          : const Offset(0, -1.18),
      end: Offset.zero,
    ).animate(curve);
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.placement == TomoToastPlacement.bottom) {
      return Positioned(
        left: 16,
        right: 16,
        bottom: TomoToast.bottomOffsetFor(widget.bottomPadding),
        child: toast,
      );
    }
    return Positioned(
      top: TomoToast.topOffsetFor(widget.topPadding),
      left: 16,
      right: 16,
      child: toast,
    );
