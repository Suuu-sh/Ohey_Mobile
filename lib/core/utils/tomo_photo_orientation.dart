...
class TomoPhotoDimensions {
  const TomoPhotoDimensions({required this.width, required this.height});
...
Future<TomoPhotoDimensions> tomoReadPhotoDimensions(String path) async {
  ...
  final dimensions = TomoPhotoDimensions(
    width: image.width,
    height: image.height,
  );
  image.dispose();
...
Future<bool> tomoIsLandscapePhoto(String path) async {
  final dimensions = await tomoReadPhotoDimensions(path);
  return dimensions.isLandscape;
}

Future<bool> tomoIsSquareOrLandscapePhoto(String path) async {
  final dimensions = await tomoReadPhotoDimensions(path);
  return dimensions.isSquareOrLandscape;
}

Future<String> tomoWriteSquarePhotoCopy(String path) async {
  return _tomoWriteCroppedPhotoCopy(
    path,
    prefix: 'square',
...
Future<String> tomoWriteLandscapePhotoCopy(String path) async {
  final dimensions = await tomoReadPhotoDimensions(path);
...
Future<String> _tomoWriteCroppedPhotoCopy(
  String path, {
  ...
  final outputPath =
      '${Directory.systemTemp.path}/tomo_${prefix}_${DateTime.now().microsecondsSinceEpoch}.png';
