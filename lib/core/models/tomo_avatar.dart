...
class TomoAvatar {
  ...
  static const defaultAvatar = TomoAvatar(
    skin: 2,
    hair: 1,
    shirt: 0,
    eyes: 0,
  ...
  /// Mascot avatar used only for official Tomo posts.
  static const adminAvatar = TomoAvatar(
    skin: 5,
    hair: 5,
    shirt: 9,
    eyes: 2,
  ...
  static TomoAvatar random({TomoGender gender = TomoGender.unspecified}) {
    final random = Random();
  ...
  String encode() => isAdmin
      ? 'tomo_avatar:admin:v1'
      : 'tomo_avatar:v2:$skin:$hair:$shirt:$eyes:$mouth:$accessory:$background';

  static TomoAvatar? decode(String? value, {bool allowAdmin = false}) {
    if (value == 'tomo_avatar:admin:v1') {
      return allowAdmin ? adminAvatar : null;
    }
    if (value == null ||
        (!value.startsWith('tomo_avatar:v1:') &&
            !value.startsWith('tomo_avatar:v2:'))) {
      return null;
    }
  ...
  TomoAvatar normalizedForGender(TomoGender gender) {
  ...
  static const backgroundStyles = ['Tomo pink'];
  ...
  static List<int> selectableHairIndicesForGender(TomoGender gender) =>
      switch (gender) {
        TomoGender.male => maleHairIndices,
        TomoGender.female => femaleHairIndices,
