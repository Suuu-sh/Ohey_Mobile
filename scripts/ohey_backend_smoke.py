#!/usr/bin/env python3
"""Backend smoke test for Ohey dev/prod environments.
...
def main() -> int:
    ...
    parser.add_argument("--invite", action="store_true", help="also create/accept an invite using OHEY_SMOKE_OTHER_* credentials")
    args = parser.parse_args()

    backend_url = env("OHEY_BACKEND_URL").rstrip("/")
    supabase_url = env("SUPABASE_URL").rstrip("/")
    publishable_key = env("SUPABASE_PUBLISHABLE_KEY")
    email = env("OHEY_SMOKE_EMAIL")
    password = env("OHEY_SMOKE_PASSWORD")

    token, user_id = sign_in(supabase_url, publishable_key, email, password)
    auth_headers = {"Authorization": f"Bearer {token}", "X-Ohey-User-ID": user_id}
    ...
    if args.invite:
        ...
        other_password = env("OHEY_SMOKE_OTHER_PASSWORD")
        ...
        if isinstance(invite, dict) and invite.get("id"):
            other_headers = {"Authorization": f"Bearer {other_token}", "X-Ohey-User-ID": other_user_id}
            expect(
                checks,
                "PATCH /v1/invites/{id}",
