#!/usr/bin/env python3
"""Backend smoke test for Nomo dev/prod environments.
...
def main() -> int:
    ...
    parser.add_argument("--invite", action="store_true", help="also create/accept an invite using NOMO_SMOKE_OTHER_* credentials")
    args = parser.parse_args()

    backend_url = env("NOMO_BACKEND_URL").rstrip("/")
    supabase_url = env("SUPABASE_URL").rstrip("/")
    publishable_key = env("SUPABASE_PUBLISHABLE_KEY")
    email = env("NOMO_SMOKE_EMAIL")
    password = env("NOMO_SMOKE_PASSWORD")

    token, user_id = sign_in(supabase_url, publishable_key, email, password)
    auth_headers = {"Authorization": f"Bearer {token}", "X-Nomo-User-ID": user_id}
    ...
    if args.invite:
        other_email = env("NOMO_SMOKE_OTHER_EMAIL")
        other_password = env("NOMO_SMOKE_OTHER_PASSWORD")
        ...
        if isinstance(invite, dict) and invite.get("id"):
            other_headers = {"Authorization": f"Bearer {other_token}", "X-Nomo-User-ID": other_user_id}
            expect(
                checks,
                "PATCH /v1/invites/{id}",
